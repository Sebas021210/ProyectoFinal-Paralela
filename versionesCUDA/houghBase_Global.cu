...
__global__ void GPU_HoughTran(unsigned char *pic, int w, int h, int *acc, float rMax, float rScale, float *d_Cos, float *d_Sin) {
    ...
    if (pic[gloID] > 0) {
        for (int tIdx = 0; tIdx < degreeBins; tIdx++) {
          // Usar d_Cos y d_Sin desde memoria global
    ...
    // Copiar resultados de la memoria compartida al acumulador global
    for (int i = locID; i < degreeBins * rBins; i += blockDim.x) {
...
void drawDetectedLines(cv::Mat &image, int *acc, int w, int h, float rMax, float rScale, int threshold) {
    ...
    for (const auto& line : strong_lines) {
      // Color rojo con 50% de transparencia
...
int main(int argc, char **argv) {
    ...
    // Copiar valores de senos y cosenos a la memoria global del device
    ...
    GPU_HoughTran<<<blockNum, 256, degreeBins * rBins * sizeof(int)>>>(d_in, w, h, d_hough, rMax, rScale, d_Cos, d_Sin);
